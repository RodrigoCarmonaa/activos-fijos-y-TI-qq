...
const allScreens = [
  ...
  { id: 8, name: "Usuarios", icon: Users, color: "emerald" },
]
...
export function AppClient({ sessionUser, initialDbState }: { sessionUser: any, initialDbState?: any }) {
  const { currentScreen, setCurrentScreen, assets, currentAssetId, setSessionUser } = useAppStore()

  // Sincronizar el usuario de sesión y el estado de la BD con el store global al cargar
  useEffect(() => {
    setSessionUser(sessionUser)
    
    if (initialDbState) {
      useAppStore.setState({
        assets: initialDbState.assets || [],
        users: initialDbState.users || [],
        custodyActs: initialDbState.custodyActs || [],
        supportTickets: initialDbState.supportTickets || [],
        auditLog: initialDbState.auditLog || [],
        software: initialDbState.software || [],
      })
    }
  }, [sessionUser, setSessionUser, initialDbState])
  ...
  const userScreens = screensByRole[currentUser.role as UserRole] || [0]
  const visibleScreens = allScreens.filter((s) => userScreens.includes(s.id))
  const roleInfo = roleLabels[currentUser.role as UserRole]
